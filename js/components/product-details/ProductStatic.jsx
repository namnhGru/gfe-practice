...
export default function ProductStatic() {
    ...
    return (
        <div className="flex flex-col">
            ...
            <div className="font-medium text-[1.875rem]/[2.25rem] text-[#525252] flex gap-[0.5rem] mb-[0.5rem]">
                <h1>{`$${sale_price[0]}`}</h1>
                {discount_percentage[0] && <span className="text-[#a3a3a3] text-[1.125rem]/[1.75rem] self-end line-through">{`$${list_price[0]}`}</span>}
            </div>
            {discount_percentage[0] && <div className="mb-[0.75rem] px-[10px] py-[4px] border-solid border-[#fde68a] border-[1px] rounded-[9999px] bg-[#fffbeb] w-fit
            ...
            <div>
                <p className="font-normal text-[1rem]/[1.5rem] text-[#525252]">{description}</p>
            </div>
        </div>
    )
}